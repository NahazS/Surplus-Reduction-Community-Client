...
const RequestCard = ({food}) => {
  const {donatorName,donatorEmail,foodName,foodImage,location,expDate, requestDate} = food
  return (
    <div>
      <tr className="bg-[#f3f7fc] bg-opacity-50 flex flex-col sm:flex-row justify-between mb-[28px] items-center px-4 py-4 h-fit w-full rounded-lg shadow-md hover:shadow-lg">
        ...
        <td>
            <div className="flex flex-col text-center sm:text-left">
                ...
                <p className="flex flex-col sm:flex-row">Request Date: <span>{requestDate}</span></p>
            </div>
        </td>
        <td><button className="btn bg-primary text-white hover:text-black">contact</button></td>
      </tr>
    </div>
  );
};
