  ...
  return (
    <div className="bg-[#f1f1f1] bg-opacity-50 text-black text-opacity-75">
        <div className="flex justify-between items-center py-6 px-5 xl:px-0 xl:max-w-[1170px] mx-auto">
            ...
            <ul className="text-lg gap-x-6 hidden lg:flex">
            ...
            <div className="flex items-center gap-x-3">
                ...
                <div>
                  <button onClick={()=> setMenu(!menu)} className="flex lg:hidden px-4 btn btn-primary"><HiOutlineMenu /></button>
                  <div style={{ zIndex: 50 }} className={`px-6 bg-white text-center absolute top-20 right-6 rounded-2xl ${menu ? 'hidden' : 'inline-flex'} `}>
                    <ul className="text-lg gap-x-6">
                      ...
                      <li onClick={() => setMenu(!menu)}><Link to={'/availableFood'}>Available Food</Link></li>
                      <li onClick={() => setMenu(!menu)}><Link to={'/addFood'}>Add Food</Link></li>
                      <li onClick={() => setMenu(!menu)}><Link to={'/myFood'}>My Food</Link></li>
                      <li onClick={() => setMenu(!menu)}><Link to={'/myFoodRequest'}>My Food Request</Link></li>
                      <li onClick={() => setMenu(!menu)}><NavLink to={'/signIn'}>Sign In</NavLink></li>
                      <li onClick={() => setMenu(!menu)} className="sm:hidden"><Link to={!user && '/signIn'} onClick={handleSignOut}>{user ? 'Sign Out' : 'Sign in'}</Link></li>
                    </ul>
                  </div>
                </div>
            </div>
